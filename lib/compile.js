...
function verHandler( type ){
    return through.obj(function (file, enc, cb) {
        var fileName = file.revOrigPath.replace(/^(.*)[\\\/]+/,""),
            filePath = file.base.replace(path.join(file.cwd,type),"").replace(/\\/,"\/").replace(/^[\\\/]?/,"");
...
function verManifest(){
    return rev.manifest(verFileName,{base:verPath,merge:false});
}
...
function compileJS( arr, type, watch ){
    ...
    objArr.forEach(function(v,i){
        ...
        gulp.src( v.source )
            ...
            .pipe( noversion ? noop() : tagHandler( type ) )
